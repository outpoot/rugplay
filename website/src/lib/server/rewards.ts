...
const MIN_REWARD_DAY_1 = 1500;
const MAX_REWARD_DAY_10_PLUS = 15000;
...
function calculateReward(streak: number, prestigeLevel: number = 0): { total: number; base: number; prestigeBonus: number } {
    const clampedStreak = Math.min(streak, 10);

    const minReward = MIN_REWARD_DAY_1 + (clampedStreak > 1 ? (clampedStreak - 1) * 100 : 0);
    const maxReward = 2500 + (clampedStreak > 1 ? (clampedStreak - 1) * 1389 : 0);
    const base = Math.floor(Math.random() * (maxReward - minReward + 1)) + minReward;
