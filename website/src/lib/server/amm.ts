...
export async function calculate24hMetrics(coinId: number, currentPrice: number) {
	...
	const [priceData] = await db
		...
		.from(priceHistory)
		.where(and(eq(priceHistory.coinId, coinId), gte(priceHistory.timestamp, twentyFourHoursAgo)))
	...
	const volumeData = await db
		...
		.from(transaction)
		.where(and(eq(transaction.coinId, coinId), gte(transaction.timestamp, twentyFourHoursAgo)));
...
export async function executeSellTrade(tx: any, coinData: any, userId: number, quantity: number) {
