<script lang="ts">
	...
	let spinningNumber = $state(0);
	let spinAnimationInterval: ReturnType<typeof setInterval>;
	...
	async function claimReward() {
		...
		spinAnimationInterval = setInterval(() => {
			spinningNumber = Math.floor(Math.random() * 15000) + 1000;
		}, 50);
		...
		try {
			...
			if (response.ok) {
				...
				spinningNumber = result.rewardAmount;
...
>
	{#if !rewardStatus || claimState === 'loading'}
		...
		<span>{!rewardStatus ? 'Loading...' : 'Spinning...'}</span>
		{#if claimState === 'loading'}
			<span class="font-mono text-sm">${spinningNumber.toLocaleString()}</span>{/if}
	{:else if claimState === 'success'}
		<CheckIcon class="h-4 w-4" />
		<span>Claimed ${spinningNumber.toLocaleString()}!</span>
