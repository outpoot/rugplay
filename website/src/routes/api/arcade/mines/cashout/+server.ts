...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (game.userId !== userId) return json({ error: 'Unauthorized: Session belongs to another user' }, { status: 403 });
    ...
    const result = await db.transaction(async (tx) => {
      ...
      return { newBalance, payout, amountWagered: game.betAmount, isAbort: false, minePositions: game.minePositions || [], revealedTiles: game.revealedTiles || [], mineCount: game.mineCount || 0 };
    });
    ...
    if (!result.isAbort) {
      ...
      try {
        await checkAndAwardAchievements(userId, ['arcade'], { arcadeWon: result.payout > result.amountWagered, arcadeWager: result.amountWagered, minesTilesRevealed: result.revealedTiles?.length ?? 0, minesCount: result.mineCount ?? 0 });
      } catch (e) {
